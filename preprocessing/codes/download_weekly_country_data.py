...
if (is_valid_start == True) and (is_valid_end == True):
    ...
    if args.ct == True:
        time_df.apply(lambda x: download_file('country2ct', x.year, x.month, x.day, output_folder), axis=1)
    if args.county == True:
        time_df.apply(lambda x: download_file('country2county', x.year, x.month, x.day, output_folder), axis=1)
    if args.state == True:
        time_df.apply(lambda x: download_file('country2state', x.year, x.month, x.day, output_folder), axis=1)
